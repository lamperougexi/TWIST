...
class G1MimicDistill(HumanoidMimic):
    ...
    def _apply_locked_dof_reference(self, dof_pos, dof_vel):
    ...
    def _reset_ref_motion(self, env_ids, motion_ids=None):
        ...
        dof_pos, dof_vel = self._apply_locked_dof_reference(dof_pos, dof_vel)
        ...
        if getattr(self, '_use_cmg', False):
            ...
            motion_key_body_ids = torch.as_tensor(self._key_body_ids_motion, dtype=torch.long, device=body_pos.device)
            global_body_pos = convert_to_global_root_body_pos(root_pos=root_pos, root_rot=root_rot, body_pos=body_pos[:, motion_key_body_ids, :])
    ...
    def compute_observations(self):
        ...
        priv_info = torch.nan_to_num(priv_info, nan=0.0, posinf=0.0, neginf=0.0)
        ...
        priv_obs_buf = torch.nan_to_num(priv_obs_buf, nan=0.0, posinf=0.0, neginf=0.0)
