...
from .angular_momentum_utils import compute_orbital_angular_momentum
...
class HumanoidMimic(HumanoidChar):
    ...
    def check_termination(self):
        ...
        if self._pose_termination:
            pose_body_ids = self._key_body_ids
            if getattr(self.cfg.env, "lower_body_only_tracking", False) and self._key_body_ids.shape[0] >= 6:
                pose_body_ids = self._key_body_ids[[2, 3, 4, 5]]
            body_pos = self.rigid_body_states[:, pose_body_ids, 0:3] - self.rigid_body_states[:, 0:1, 0:3]
            tar_body_pos = self._ref_body_pos[:, pose_body_ids] - self._ref_root_pos[:, None, :] 
    ...
    def compute_observations(self):
        ...
        obs_buf = torch.nan_to_num(obs_buf, nan=0.0, posinf=0.0, neginf=0.0)
        priv_latent = torch.nan_to_num(priv_latent, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    def _reward_tracking_keybody_pos(self):
        lower_body_only = getattr(self.cfg.env, "lower_body_only_tracking", False)
        # Lower body key body indices within key_bodies list: 2,3 (ankles), 4,5 (knees)
        if (lower_body_only or (hasattr(self, '_use_cmg') and self._use_cmg)) and self._key_body_ids.shape[0] >= 6:
    ...
    def _reward_total_angular_momentum(self):
        body_pos = self.rigid_body_states[:, :, 0:3]
        body_vel = self.rigid_body_states[:, :, 7:10]
        ang_momentum = compute_orbital_angular_momentum(body_pos, body_vel, self.rigid_body_masses)
        total_mass = self.rigid_body_masses.sum(dim=1, keepdim=True).clamp(min=1e-6)
        normalized_ang_momentum = ang_momentum / total_mass
        return torch.sum(torch.square(normalized_ang_momentum), dim=1)
    ...
    def _reward_tracking_keybody_pos_upper(self):
        ...
        """
        if getattr(self.cfg.env, "lower_body_only_tracking", False):
