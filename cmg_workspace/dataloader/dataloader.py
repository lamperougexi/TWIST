...
def preprocess_amass_for_cmg(
    ...
    for npz_path in tqdm(npz_files, desc="Processing"):
        ...
        try:
            ...
            for start in range(0, T - seq_len, seq_len // 2):
                ...
                motion_seg = motion_clean[start:end+1]  # [seq_len+1, 58]
                command_seg = command[start:end].astype(np.float32)  # [seq_len, 3]

                # Up-sampling: More balanced distribution
                # Upweight high-speed and very slow, but not as extreme
                if segment_speed > 1.8:
                    repeat = 36
                elif segment_speed > 1.4:
                    repeat = 24   # Fast walking/jogging
                elif segment_speed > 0.6:
                    repeat = 6  # Normal walking
    ...
    motion_std_raw = all_motion.std(axis=0)
